...
sum_probs_0_to_9 <- sum(apgar_probs[1:10], na.rm = TRUE)

p10 <- 1 - sum_probs_0_to_9
...
mean_apgar <- sum(apgar_vals * apgar_probs)
...
mean_square_apgar <- sum((apgar_vals^2) * apgar_probs)

variance_apgar <- mean_square_apgar - (mean_apgar^2)
...
std_dev_apgar <- sqrt(variance_apgar)
...
> discrete, becuase the number of students is a countable value that can only take whole numbers, so it is trated as discrete. 
...
> Continuous, because even though individual ages are whole numbers, the average age can take any value within a range, so it is treated as continuous.
...
> Continuous, because even though the length is rounded to the nearest second, time can take any value within a range, so it is treated as continuous.
...
ggplot(lectures, aes(x = total_minutes)) +
  ...
  scale_x_continuous(breaks = seq(47, 52, 1)) 
  labs(title = "Histogram of Lecture Lengths", x = "Total Minutes", y = "Frequency") +
  theme_minimal()  
...
lectures_summary <- lectures %>%
  group_by(base_minutes) %>%
  summarize(n = n()) %>%
  mutate(percent = (n / sum(n)) * 100) 

lectures_summary

percent_50_minutes <- lectures_summary %>%
  filter(base_minutes == 50) %>%
  pull(percent)
...
> No, because 55.3% is based on past data, and doesn't predict the exact probability for a future event.
...
> Yes, it is valid because the 55.3% represents the proportion of past lectures that were 50 minutes long, so selecting a random lecture from that set means the probability reflects the observed data.
...
> 
No, the original model may not seem reasonable anymore because it predicted that a 42-minute lecture is extremely unlikely, yet it occurred. This suggests that the model might not fully capture the variability or factors affecting lecture durations, so adjustments may be needed to account for outliers like this one.
...
> No, because 36.8% reflects how often he wore navy shirts in lectures, not how many he owns in total.
