...
```{r}
ggplot(monona, aes(x = "", y = duration)) +
  geom_boxplot(fill = "skyblue", color = "navyblue") +
  labs(title="Monona's Closure Duration")
```
...
```{r}
ggplot(monona, aes(x = ff_x, y = duration, color=decade)) +
  geom_point() +
  labs(title="Monona's Closure Duration") +
  theme_minimal()
```
...
ggplot(monona, aes(x = ff_x, y = duration, color=decade)) +
  geom_point() +
  geom_smooth(se = F, linetype = "dashed", color="red") +
  labs(title="Monona's Closure Duration") +
  theme_minimal()
```
...
```{r}
ggplot(monona, aes(x = ff_x, y = duration, color=as_factor(decade))) +
  geom_point() +
  labs(title="Monona's Closure Duration") +
  theme_minimal()
```
...
> Yes, the decades are more clear on which is which, this is simply the better version of the gradient.
...
```{r}
ggplot(monona, aes(x = year1)) +
  geom_ribbon(aes(ymin=ff_x, ymax=lt_x)) +
  labs(title="Monona's Closure Duration") +
  theme_minimal()
```
